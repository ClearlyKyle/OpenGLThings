...
vec4 pointLight()
{
    ...
    return (texture(tex0, texCoord) * (diffuse * inten + ambient) +
            texture(tex1, texCoord).r * specular * inten) *
           lightColor;
}
...
vec4 spotLight()
{
    ...
    // calculates the intensity of the crntPos based on its angle to the center of
    // the light cone
    float angle = dot(vec3(0.0f, -1.0f, 0.0f), -lightDirection);
    float inten =
        clamp((angle - outerCone) / (innerCone - outerCone), 0.0f, 1.0f);
